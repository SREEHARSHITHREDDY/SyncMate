...
export default function AvailabilityFinder() {
  ...
  // FIX 12: when a friend is selected, fetch their events from the DB
                ...
                <div className="grid gap-1 mb-1" style={{ gridTemplateColumns: `52px repeat(${DAYS_AHEAD}, 1fr)` }}>
                  ...
                  {days.map((day) => (
                    <div key={day.toISOString()} className="text-center px-1">
                      ...
                      <div className={`text-base font-bold leading-none mt-0.5 ${isToday(day) ? "text-primary" : isWeekend(day) ? "text-muted-foreground" : ""}`}>
                        {format(day, "d")}
                      </div>
                              ...
                              <button
                                ...
                                className={`h-7 rounded-sm transition-all border text-[10px] font-medium w-full ${
                                  status === "free"
                                    ...
                                    : status === "my-busy"
                                    ? "bg-amber-400/15 border-amber-400/25 cursor-not-allowed"
