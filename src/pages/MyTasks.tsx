...
export default function MyTasks() {
  ...
  // FIX: use a ref to track mounted state so async callbacks
  // don't call setState after the component unmounts (which causes the crash)
  ...
  const getDueDateInfo = (dueDate: string | null) => {
    ...
    return { label, isOverdue, isUrgent: daysUntil <= 1 && daysUntil >= 0, isToday: isToday(date) };
  };
  ...
  const filteredItems = localItems.filter(item => {
    if (searchQuery && !item.content.toLowerCase().includes(searchQuery.toLowerCase()) &&
      !item.event_title?.toLowerCase().includes(searchQuery.toLowerCase())) return false;
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      ...
      if (isMountedRef.current) toast.success("Task order updated");
    } catch {
  ...
  // FIX: wrap in useCallback so they are stable references for the useEffect below
  const handleBulkComplete = useCallback(async () => {
    ...
    try {
      ...
      if (isMountedRef.current) toast.error(error.message || "Failed to complete tasks");
    } finally {
      if (isMountedRef.current) setProcessingIds(new Set());
    }
  }, [selectedItems, user?.id, queryClient]);
  ...
  const handleBulkRestore = useCallback(async () => {
    ...
    try {
      ...
      if (isMountedRef.current) toast.error(error.message || "Failed to restore tasks");
    } finally {
      if (isMountedRef.current) setProcessingIds(new Set());
    }
  }, [selectedItems, user?.id, queryClient]);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      if (isMountedRef.current) toast.error(error.message || "Failed to delete tasks");
    }
  };
  ...
  const handleToggleComplete = async (item: UserActionItem) => {
    ...
    try {
      ...
      if (isMountedRef.current) toast.error(error.message || "Failed to complete task");
    } finally {
      if (isMountedRef.current) setProcessingIds(prev => { const next = new Set(prev); next.delete(item.id); return next; });
    }
  };
  ...
  const handleRestoreItem = async (item: UserActionItem) => {
    ...
    try {
      ...
      if (isMountedRef.current) toast.error(error.message || "Failed to restore task");
    } finally {
      if (isMountedRef.current) setProcessingIds(prev => { const next = new Set(prev); next.delete(item.id); return next; });
    }
  };
  ...
  // FIX: The original useEffect was missing handleBulkComplete and handleBulkRestore
  // in its dependency array. This meant stale closures were capturing old state
  // and when navigating away, the cleanup didn't work properly — React crashed
  // because setState was called on an unmounted component.
  // Solution: wrap handlers in useCallback (done above) and include them in deps.
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (!isMountedRef.current) return; // guard: don't run if unmounted
      ...
      if ((e.key === "Delete" || e.key === "Backspace") && selectedItems.size > 0 && !isInput) {
        e.preventDefault();
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === "Enter" && selectedItems.size > 0) {
        e.preventDefault();
        if (activeTab === "active") handleBulkComplete();
        else handleBulkRestore();
      }
    };

    window.addEventListener("keydown", handleKeyDown);
    // FIX: cleanup runs correctly now because all referenced values are in deps
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [toggleSelectAll, clearSelection, selectedItems, activeTab, handleBulkComplete, handleBulkRestore]);
      ...
      <div className="container py-8 space-y-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Manage all your action items across events</p>
          </div>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    >
                      ...
                      <ToggleGroupItem
                        value="overdue" size="sm"
                        className={cn("text-xs gap-1.5 px-3", overdueCount > 0 && "data-[state=on]:bg-destructive data-[state=on]:text-destructive-foreground")}
                        disabled={overdueCount === 0}
                      >
                        <AlertTriangle className="h-3.5 w-3.5" />
                        Overdue ({overdueCount})
                      </ToggleGroupItem>
                      <ToggleGroupItem value="today" size="sm" className="text-xs gap-1.5 px-3" disabled={dueTodayCount === 0}>
                        <CalendarIcon className="h-3.5 w-3.5" />
                        Due Today ({dueTodayCount})
                      </ToggleGroupItem>
                      <ToggleGroupItem value="upcoming" size="sm" className="text-xs gap-1.5 px-3" disabled={upcomingCount === 0}>
                        <Clock className="h-3.5 w-3.5" />
                        Upcoming ({upcomingCount})
                      </ToggleGroupItem>
                      <ToggleGroupItem value="no-date" size="sm" className="text-xs gap-1.5 px-3" disabled={noDateCount === 0}>
                        No Date ({noDateCount})
                      </ToggleGroupItem>
                    </ToggleGroup>
                    ...
                    <div className="flex items-center gap-2 ml-auto">
                      ...
                      {sortBy !== "custom" && (
                        <Button variant="outline" size="sm" onClick={() => setSortDirection(prev => prev === "asc" ? "desc" : "asc")} className="h-8 w-8 p-0">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Button variant={priorityFilter === "all" ? "secondary" : "ghost"} size="sm" onClick={() => setPriorityFilter("all")} className="h-6 text-xs">All</Button>
                  ...
                  {allTags.length > 0 && (
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Button variant={tagFilter === null ? "secondary" : "ghost"} size="sm" onClick={() => setTagFilter(null)} className="h-6 text-xs">All</Button>
                      {allTags.map(tag => (
                        <Button
                          key={tag}
              ...
              <Card className="border-primary/50 bg-primary/5 animate-fade-in">
                <CardContent className="py-3">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <span className="text-sm font-medium">{selectedItems.size} task(s) selected</span>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Button variant="default" size="sm" onClick={handleBulkComplete} disabled={processingIds.size > 0} className="gap-1.5">
                        <CheckCircle2 className="h-4 w-4" />
                        Complete
                      </Button>
                      <Button variant="destructive" size="sm" onClick={() => setShowBulkDeleteDialog(true)} className="gap-1.5">
          ...
          <TabsContent value="completed" className="space-y-4">
              ...
              <Card className="border-primary/50 bg-primary/5 animate-fade-in">
                <CardContent className="py-3">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      <Button variant="outline" size="sm" onClick={() => setSelectedItems(new Set())}>Clear</Button>
                      <Button variant="default" size="sm" onClick={handleBulkRestore} disabled={processingIds.size > 0} className="gap-1.5">
            ...
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription>{sortedItems.length} task{sortedItems.length !== 1 ? "s" : ""} shown</CardDescription>
              </CardHeader>
              <CardContent>
                {isLoading ? (
                  <div className="flex items-center justify-center py-12 text-muted-foreground">Loading tasks...</div>
                    ...
                    {sortedItems.map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className={cn(
                            "flex items-start gap-3 p-4 rounded-lg border bg-card transition-all group hover:bg-accent/30",
                            isSelected && "ring-2 ring-primary/50"
                          )}
                        >
                          <Checkbox checked={isSelected} onCheckedChange={() => toggleSelectItem(item.id)} className="mt-1" />
                          <div className="flex-1 min-w-0">
                            <p className="font-medium leading-tight line-through text-muted-foreground">{item.content}</p>
                            <div className="flex items-center gap-2 mt-2 flex-wrap">
                              <Link to="/calendar" className="text-xs text-muted-foreground hover:text-primary transition-colors flex items-center gap-1">
...
function SortableTaskItem({ item, isSelected, isProcessing, isDraggable, onToggleSelect, onComplete, onEdit, getDueDateInfo }: SortableTaskItemProps) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: item.id, disabled: !isDraggable });
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className={cn("font-medium leading-tight", isProcessing && "line-through text-muted-foreground")}>
            {item.content}
          </p>
          <Tooltip>
        ...
        <div className="flex items-center gap-2 mt-2 flex-wrap">
          ...
          {!item.due_date && <Badge variant="outline" className="text-xs px-2 py-0.5">No due date</Badge>}
          {item.recurrence_type && (
            <Badge variant="secondary" className="text-xs px-2 py-0.5 gap-1">
      ...
      <div className="flex items-center gap-1 shrink-0">
        <Tooltip>
          <TooltipTrigger asChild>
            <Button variant="ghost" size="sm" onClick={onEdit} className="h-8 w-8 p-0 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button variant="ghost" size="sm" onClick={onComplete} disabled={isProcessing} className="gap-1.5">
              {isProcessing
                ? <CheckCircle2 className="h-4 w-4 animate-pulse text-primary" />
                : <><CheckCircle2 className="h-4 w-4" /><span className="hidden sm:inline">Complete</span></>
              }
            </Button>
          </TooltipTrigger>
